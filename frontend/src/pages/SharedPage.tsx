...
export default function SharedPage() {
  ...
  return (
    <div>
      <Topbar />
      <h1 className="px-6 pb-3 pt-2 text-[22px] text-g-text dark:text-gray-100">Shared with me</h1>
      {shared.isLoading ? (
          ...
          {(shared.data ?? []).map((i) => {
            ...
            return (
              <li key={`${i.item_type}-${i.id}`} className="flex items-center gap-3 rounded-lg border border-g-border px-4 py-2.5 hover:bg-g-hover dark:border-white/10 dark:hover:bg-white/5">
                ...
                <button className="flex-1 truncate text-left text-sm text-g-text dark:text-gray-100" onClick={() => openItem(i)}>
                  {i.name}
                </button>
                <span className="text-sm text-g-muted dark:text-gray-400">{i.role} · {i.owner_email}</span>
              </li>
            );
          })}
        </ul>
