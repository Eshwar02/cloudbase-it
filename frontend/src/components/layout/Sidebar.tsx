...
export function Sidebar() {
  ...
  return (
    <aside className="glass m-3 flex w-56 flex-col gap-2 rounded-xl2 p-4">
      ...
      <NavLink to="/shared" className={({ isActive }) => `${link} ${isActive ? active : idle}`}>Shared</NavLink>
      <NavLink to="/starred" className={({ isActive }) => `${link} ${isActive ? active : idle}`}>Starred</NavLink>
      <NavLink to="/search" className={({ isActive }) => `${link} ${isActive ? active : idle}`}>Search</NavLink>
